...
size_t node_innit_frame_allocator(ProgramContext *ctxt,
                                  AllocatorInterface *allocator);
size_t node_innit_bucket_allocator(ProgramContext *ctxt,
                                   AllocatorInterface *allocator);
size_t node_innit_event_loop(ProgramContext *ctxt,
                             AllocatorInterface *allocator);
size_t node_innit_deadline(ProgramContext *ctxt, AllocatorInterface *allocator);
size_t node_innit_io(ProgramContext *ctxt, AllocatorInterface *allocator);
size_t node_innit_metrics(ProgramContext *ctxt, AllocatorInterface *allocator);
void node_init_ctxt(ProgramContext *ctxt, AllocatorInterface *allocator) {

  size_t total_expected = 0;
  const size_t expected_frame_allocator_size =
      node_innit_frame_allocator(ctxt, allocator);
  ...
  const size_t expected_bucket_allocator_size =
      node_innit_bucket_allocator(ctxt, allocator);
  ...
  const size_t expected_event_loop_size =
      node_innit_event_loop(ctxt, allocator);
  ...
  const size_t expected_deadline_size = node_innit_deadline(ctxt, allocator);
  ...
  const size_t expected_io_size = node_innit_io(ctxt, allocator);
  ...
  const size_t expected_metric_size = node_innit_metrics(ctxt, allocator);
  ...
  ctxt->logger->log_info(
      NODE_TAG, "Metric initialized, expected size: [%lu], actual size: [%lu]",
      expected_metric_size, actual_metric_size);

  ctxt->logger->log_info(NODE_TAG,
...
size_t node_innit_deadline(ProgramContext *ctxt,
                           AllocatorInterface *allocator) {
