...
BusyPollingSerialTransport::io_open(const IOAddress *host_addr) {
  auto self_ctxt = co_await get_ctxt();
  self_ctxt->set_name(NAME_IO_SERIAL_OPEN);
  self_ctxt->trace.start();

  int res = co_await busy_polling_awaiters::IOSerialOpenAwaiter(
      program_ctxt->name_lookup->get_name(host_addr->val.file_name_index));

  co_return process_io_res(self_ctxt, IOMethod::IO_SERIAL, IOType::OPEN, res);
}

Task<std::expected<int, ErrorWrapper>> BusyPollingSerialTransport::io_write(
    const IOAddress *host_addr, const uint8_t *buf, uint64_t buf_size,
    IOPackageType package_type, bool with_ack) {
  auto self_ctxt = co_await get_ctxt();
  self_ctxt->set_name(NAME_IO_SERIAL_WRITE);
  self_ctxt->trace.start();

  int res = co_await busy_polling_awaiters::IOSerialWriteAwaiter(
...
Task<std::expected<int, ErrorWrapper>>
BusyPollingSerialTransport::io_write(const IOAddress *host_addr,
...
Task<std::expected<int, ErrorWrapper>>
BusyPollingSerialTransport::io_read(const IOAddress *host_addr, uint8_t *buf,
                                    uint64_t buf_size) {
  auto self_ctxt = co_await get_ctxt();
  self_ctxt->set_name(NAME_IO_SERIAL_READ);
  self_ctxt->trace.start();

  int res = co_await busy_polling_awaiters::IOSerialReadAwaiter(
      host_addr->val.fd, buf, buf_size);
  co_return process_io_res(self_ctxt, IOMethod::IO_SERIAL, IOType::READ, res);
}

Task<std::expected<int, ErrorWrapper>>
BusyPollingSerialTransport::io_close(const IOAddress *host_addr) {
  auto self_ctxt = co_await get_ctxt();
  self_ctxt->set_name(NAME_IO_SERIAL_CLOSE);
  self_ctxt->trace.start();
  int res =
      co_await busy_polling_awaiters::IOSerialCloseAwaiter(host_addr->val.fd);
