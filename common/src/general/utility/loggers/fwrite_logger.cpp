...
uint64_t FWriteLogger::print_tag(const char *prefix, const char *type_text,
...
void FWriteLogger::log_general_v(const char *prefix, const char *type_text,
...
FWriteLogger::log_info(const char *tag, const char *fmt, ...) noexcept {
  va_list args;
  va_start(args, fmt);
...
__attribute__((format(printf, 3, 4))) void
FWriteLogger::log_debug(const char *tag, const char *fmt, ...) noexcept {
  if (!DEBUG_ON) {
    return;
  }
  va_list args;
...
__attribute__((format(printf, 3, 4))) void
FWriteLogger::log_warning(const char *tag, const char *fmt, ...) noexcept {
  va_list args;
  va_start(args, fmt);
...
__attribute__((format(printf, 3, 4))) void
FWriteLogger::log_err(const char *tag, const char *fmt, ...) noexcept {
  va_list args;
  va_start(args, fmt);
