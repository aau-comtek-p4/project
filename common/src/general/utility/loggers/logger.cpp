...
LogEntry logging::log_io_timeout(IOMethod io_method, IOType io_type,
  ...
  entry.payload.io_timeout.name_index = name_index;
  entry.payload.io_timeout.trace_index = trace_index;
  return entry;
}
...
LogEntry logging::log_io_complete(IOMethod io_method, IOType io_type,
  ...
  entry.payload.io_completed.name_index = name_index;
  entry.payload.io_completed.trace_index = trace_index;
  return entry;
}
...
LogEntry logging::log_io_error(IOMethod io_method, IOType io_type,
  ...
  entry.payload.io_error.name_index = name_index;
  entry.payload.io_error.trace_index = trace_index;
  return entry;
}
...
uint64_t JsonLogSerializer::serialize(char *buf, uint64_t max_entry,
  ...
  case LogReason::REASON_IO_COMPLETED:
    return snprintf(buf + header_size, max_entry - header_size,
                    ...
                    program_ctxt->name_lookup->get_name(
                        log_entry.payload.io_completed.name_index),
                    program_ctxt->name_lookup->get_name(
                        log_entry.payload.io_completed.parent_name_index),
                    log_entry.payload.io_completed.trace_index,
  ...
  case LogReason::REASON_IO_TIMEOUT:
    return snprintf(buf + header_size, max_entry - header_size,
                    ...
                    program_ctxt->name_lookup->get_name(
                        log_entry.payload.io_timeout.name_index),
                    program_ctxt->name_lookup->get_name(
                        log_entry.payload.io_timeout.parent_name_index),
                    log_entry.payload.io_timeout.trace_index) +
           header_size;
  case LogReason::REASON_IO_ERROR:
    return snprintf(buf + header_size, max_entry - header_size,
                    "\"type\":\"%s\",\"method\":\"%s\",\"name\":\"%s\","
                    ...
                    program_ctxt->name_lookup->get_name(
                        log_entry.payload.io_error.name_index),
                    program_ctxt->name_lookup->get_name(
                        log_entry.payload.io_error.parent_name_index),
                    log_entry.payload.io_error.trace_index,
  ...
  case LogReason::REASON_NEW_DEADLINE:
    return snprintf(buf + header_size, max_entry - header_size,
                    "\"deadline_time_ms\":%" PRIu64 "}}\n",
                    log_entry.payload.new_deadline.absolute_deadline_ms) +
           header_size;
  case LogReason::REASON_DEBUG:
    return snprintf(buf + header_size, max_entry - header_size,
